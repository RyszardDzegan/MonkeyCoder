...
namespace MonkeyCoder.Functions.Tests
{
    ...
    public class InvocationTests
    {
        ...
        public void Works_with_call_stack()
        {
            ...
            AreSame(func, ((FunctionInvocation)e.Current).Delegate);
            AreEqual(1, e.Current.Arguments.Count());
            IsInstanceOf<FunctionInvocation>(e.Current.Arguments.First());
            AreSame(func, ((FunctionInvocation)e.Current.Arguments.First()).Value);
            AreEqual(1, ((FunctionInvocation)e.Current.Arguments.First()).Arguments.Count());
            IsInstanceOf<ValueInvocation>(((FunctionInvocation)e.Current.Arguments.First()).Arguments.First());
            AreEqual("b", ((ValueInvocation)((FunctionInvocation)e.Current.Arguments.First()).Arguments.First()).Value);
